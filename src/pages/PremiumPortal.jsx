          ...
          {tiers.map((tier) => {
            ...
            <article id={`tier-${tier.id}`} key={tier.id} className={`tier-realm tier-realm--${tier.id} ${activeMembership && Number(profile.tier) === tier.id ? "is-current" : ""}`}>
              ...
              <div className="tier-realm__content">
                ...
                <button type="button" disabled={checkoutTier !== null} onClick={() => chooseTier(tier)} className="tier-realm__button">{checkoutTier === tier.id ? "Opening secure checkout…" : activeMembership && Number(profile.tier) === tier.id ? (hasStripeBilling ? "Manage Current Dimension" : "Current Dimension") : activeMembership && hasStripeBilling ? "Change in Billing Portal" : `Enter Dimension ${tier.id}`}</button>
              </div>
            </article>
          )})}
        </section>
