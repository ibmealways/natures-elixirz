...
export default function BetaAdmin() {
  const { user, emulatorMode, emulatorStatus, signOut } = useAuth();
  ...
  return <div className="cosmic-page-shell beta-admin-cosmos"><GlowNav /><main className="beta-admin-page">
    ...
    {emulatorMode && emulatorStatus !== "ready" ? <section className="beta-lock"><ShieldCheck size={32} /><h2>Local acceptance environment not ready</h2><p>{emulatorStatus === "checking" ? "Verifying Auth, Firestore, and Functions emulators before synthetic data can be created..." : emulatorStatus}</p><small>Acceptance mode has no production fallback.</small></section> : !user ? <section className="beta-lock"><ShieldCheck size={32} /><h2>Sign in first</h2><p>Use the authorized Nature&apos;s Elixirz administrator account.</p>{emulatorMode && <small>LOCAL ACCEPTANCE MODE: Auth, Firestore, and Functions use emulators only.</small>}</section> : authorized === false ? <section className="beta-lock"><ShieldCheck size={32} /><h2>Administrator authorization required</h2><p>Create the protected Firestore document <code>betaAdmins/{user.uid}</code> with boolean field <code>enabled: true</code>, then refresh this page.</p><small>Signed-in UID: {user.uid}</small>{emulatorMode && <button onClick={() => signOut().then(() => window.location.reload())}>Clear local acceptance session</button>}</section> : <>
