...
import { evaluateNourishmentMatrix } from "../utilities/humanNourishmentMatrix";
  ...
  return <div className="cosmic-page-shell smoothie-cosmos"><GlowNav /><main className="ne-page smoothie-lab-page">
    ...
    <section className="ne-recipe-grid">
      ...
      <aside className="recipe-sidecar">
        <section className="ne-panel nourishment-matrix" aria-label="Human Nourishment Matrix assessment">
          <div className="matrix-heading"><div><p className="ne-kicker">Human Nourishment Matrix</p><h2>Biological formulation map</h2></div><div className="matrix-score"><strong>{nourishmentMatrix.score}</strong><span>/ 100</span></div></div>
          <p className="ne-muted">Ingredient → nutrient pattern → biological pathway → intended nutritional function.</p>
          <div className="matrix-system-grid">{nourishmentMatrix.systems.map((system) => <div key={system.id}><span><b>{system.label}</b><em>{system.score}</em></span><i><u style={{ width: `${system.score}%` }} /></i><small>{system.pathway}</small></div>)}</div>
          {nourishmentMatrix.synergies.length > 0 && <div className="matrix-insight"><strong>Nutrient synergy detected</strong>{nourishmentMatrix.synergies.slice(0, 3).map((item) => <p key={item}>{item}</p>)}</div>}
          {nourishmentMatrix.gaps.length > 0 && <div className="matrix-insight matrix-gap"><strong>Missing-link detector</strong>{nourishmentMatrix.gaps.slice(0, 2).map((item) => <p key={item}>{item}</p>)}</div>}
          {nourishmentMatrix.saturation && <div className="matrix-insight matrix-warning"><strong>Ingredient saturation detected</strong><p>{nourishmentMatrix.saturation}</p></div>}
          <small className="matrix-boundary">Pattern-coverage score—not a clinical adequacy score or predicted health outcome. Exact conclusions require verified quantities, identities, and product labels.</small>
        </section>
