...
import { findNutritionLabel, getNutritionLabels, restoreNutritionLabels, saveNutritionLabels, upsertNutritionLabel } from "./nutritionLabelRegistry";
...
describe("Nutrition Facts registry", () => {
  ...
  it("reuses a yogurt label for prepared Greek yogurt names", () => {
    expect(findNutritionLabel([{ ...label, ingredientName: "Yogurt" }], "Plain Greek yogurt")?.ingredientName).toBe("Yogurt");
  });
  it("does not merge distinct milk types", () => {
    expect(findNutritionLabel([{ ...label, ingredientName: "Milk" }], "Unsweetened soy milk")).toBeUndefined();
  });
  it("replaces an existing canonical ingredient record instead of creating a duplicate", () => {
    upsertNutritionLabel([], { ...label, id: "first", ingredientName: "Yogurt" }, "subscriber-a");
    const saved = upsertNutritionLabel(getNutritionLabels("subscriber-a"), { ...label, id: "second", ingredientName: "Plain Greek yogurt", brand: "Updated" }, "subscriber-a");
    expect(saved).toHaveLength(1);
    expect(saved[0]).toMatchObject({ id: "second", brand: "Updated" });
  });
});
