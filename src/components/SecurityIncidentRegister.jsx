...
import { buildIncidentRecordEntry, getVisibleIncidentRecordSections } from "../utilities/securityIncidentRecordEntry";
import { getGenericLifecycleStatusOptions, getNotificationDecisionControl } from "../utilities/incidentWorkflowUi";
...
export default function SecurityIncidentRegister() {
  ...
  async function addEntry(event) {
    ...
    const record = buildIncidentRecordEntry({ kind: entry.kind, sourceReference: entry.primary, description: entry.secondary, storageReference: entry.reference });
    await run(record.action, record.payload);
  ...
  const notificationDecisionControl = getNotificationDecisionControl(selected);
  return <section className="incident-register">
    ...
    <div className="incident-workspace"><aside>{incidents.map((item) => <button className={selected?.incidentId === item.incidentId ? "active" : ""} key={item.incidentId} onClick={() => setSelected(item)}><strong>{item.severity} · {item.status}</strong><span>{item.title}</span></button>)}</aside>
      {selected ? <article><h3>{selected.title}</h3><p>{selected.summary}</p><div className="incident-controls"><label>Status<select value={selected.status} onChange={(e) => run("transition", { nextStatus: e.target.value })}>{getGenericLifecycleStatusOptions(statuses).map(({ status, disabled }) => <option key={status} value={status} disabled={disabled}>{status}</option>)}</select></label><label>Severity<select value={selected.severity} onChange={(e) => run("update", { changes: { severity: e.target.value } })}>{["SEV0", "SEV1", "SEV2", "SEV3"].map((item) => <option key={item}>{item}</option>)}</select></label></div>
      ...
      <form className="incident-entry" onSubmit={addEntry}><select value={entry.kind} onChange={(e) => setEntry({ ...entry, kind: e.target.value })}><option value="timeline">Timeline event</option><option value="evidence">Evidence reference</option><option value="finding">Technical finding</option></select><input required placeholder="Source / reference" value={entry.primary} onChange={(e) => setEntry({ ...entry, primary: e.target.value })} /><input required placeholder="Minimized description" value={entry.secondary} onChange={(e) => setEntry({ ...entry, secondary: e.target.value })} />{entry.kind === "evidence" && <input placeholder="Storage reference only (no copied health data)" value={entry.reference} onChange={(e) => setEntry({ ...entry, reference: e.target.value })} />}<button disabled={busy}>Add record</button></form>
      <section className="incident-recorded-items" aria-label="Recorded incident items"><h4>Recorded incident items</h4>{getVisibleIncidentRecordSections(selected).map((section) => section.items.length ? <section key={section.key}><h5>{section.label}</h5><ul>{section.items.map((item, index) => <li key={`${section.key}-${index}`}><strong>{item.type}</strong><span>{item.source}</span><p>{item.description}</p>{item.storageReference && <small>Storage reference: {item.storageReference}</small>}</li>)}</ul></section> : null)}</section>
      <div className="incident-actions"><select id="containment-item">{checklist.map((item) => <option key={item}>{item}</option>)}</select><button disabled={busy} onClick={() => run("addContainment", { item: { checklistId: document.getElementById("containment-item").value, status: "IN_PROGRESS", notes: "Tracking entry only; no action executed." } })}>Track containment item</button><button disabled={busy} onClick={() => run("requestCounselReview")}>Request counsel review</button><button disabled={busy} onClick={() => run("recordCounselDecision", { decision: { status: "COUNSEL_REVIEWED", notesReference: "Restricted counsel record reference" } })}>Record counsel reviewed</button><button disabled={busy || !notificationDecisionControl.eligible} onClick={() => notificationDecisionControl.eligible && run(notificationDecisionControl.action, notificationDecisionControl.payload)}>Record decision pending</button>{!notificationDecisionControl.eligible && <p className="incident-workflow-explanation" aria-live="polite">{notificationDecisionControl.explanation}</p>}</div><section className="incident-containment" aria-live="polite"><h4>Tracked containment items</h4>{(selected.containmentActions || []).length ? <ul>{selected.containmentActions.map((item) => <li key={item.checklistId}><strong>{item.status}</strong> | {item.checklistId} <small>Recordkeeping only; no action executed.</small></li>)}</ul> : <p>No containment items are recorded yet.</p>}</section>
