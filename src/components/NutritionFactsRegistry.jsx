...
import { findNutritionLabel, getNutritionLabels, normalizeNutritionIdentity, NUTRIENT_FIELDS, saveNutritionLabels, upsertNutritionLabel } from "../utilities/nutritionLabelRegistry";
...
export default function NutritionFactsRegistry({ scope, pantryItems = [], activeIngredients = [], activeFormulaName = "" }) {
  ...
  useEffect(() => {
    const refresh = (event) => {
      if (!event?.detail?.scope || event.detail.scope === scope) setLabels(getNutritionLabels(scope));
    };
    window.addEventListener("naturesElixirz:data-changed", refresh);
    window.addEventListener("naturesElixirz:data-restored", refresh);
    return () => {
      window.removeEventListener("naturesElixirz:data-changed", refresh);
      window.removeEventListener("naturesElixirz:data-restored", refresh);
    };
  }, [scope]);
  useEffect(() => { if (activeFormulaName && registryRef.current) registryRef.current.open = true; }, [activeFormulaName]);
  const formulaReview = useMemo(() => {
    const identities = new Map();
    activeIngredients.forEach((ingredient) => {
      const name = String(ingredient?.name || ingredient || "").trim();
      if (!name) return;
      const identity = normalizeNutritionIdentity(name) || name.toLowerCase();
      const existing = identities.get(identity);
      if (existing) { existing.uses += 1; return; }
      identities.set(identity, { name, quantity: ingredient?.quantity || `${ingredient?.amount ?? ""} ${ingredient?.unit || ""}`.trim(), match: findNutritionLabel(labels, name), labelRecommended: packageLabelRecommended(name), uses: 1 });
    });
    return [...identities.values()];
  }, [activeIngredients, labels]);
  ...
  const loadIngredient = (ingredient) => {
    ...
    window.setTimeout(() => registryRef.current?.querySelector("form")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  };
  const updateNutrient = (key, value) => setForm((current) => ({ ...current, nutrients: { ...current.nutrients, [key]: value } }));
  const submit = (event) => {
    event.preventDefault();
    try {
      const saved = upsertNutritionLabel(labels, form, scope);
      const next = saved[saved.length - 1];
      setLabels(saved); setForm(emptyForm()); setFormOpen(false); setSavedLabelsOpen(false); setMessage(`${next.brand} ${next.productName} was saved and applied to matching ${next.ingredientName} occurrences across Smoothies and Meal Plans.`);
    } catch (error) { setMessage(error.message); }
  };
  const remove = (id) => { const saved = saveNutritionLabels(labels.filter((item) => item.id !== id), scope); setLabels(saved); if (!saved.length) setSavedLabelsOpen(false); setMessage("Nutrition Facts record removed."); };
  return <details ref={registryRef} className="nutrition-facts-registry">
    ...
    <div className="label-registry-body">
      ...
      {formulaReview.length > 0 && <section className="active-formula-label-review"><header><div><small>Current generated formula</small><strong>{activeFormulaName}</strong></div><span>{formulaReview.filter((item) => item.match).length} label-matched · {needsLabel.length} need package data</span></header><div>{formulaReview.map((ingredient) => <article className={ingredient.match ? "is-matched" : ingredient.labelRecommended ? "needs-label" : "reference-ready"} key={normalizeNutritionIdentity(ingredient.name) || ingredient.name}><div><strong>{ingredient.name}</strong><small>{ingredient.quantity || "Quantity pending"}{ingredient.uses > 1 ? ` · used ${ingredient.uses} times` : ""}</small></div>{ingredient.match ? <span>Label matched</span> : ingredient.labelRecommended ? <button type="button" onClick={() => loadIngredient(ingredient)}>Enter package label</button> : <span>Generic reference</span>}</article>)}</div><p>Each food appears once here even when the plan uses it repeatedly. A saved package label is reused for matching ingredient names in both Smoothies and Meal Plans. Generic produce can use a matched reference-food estimate.</p></section>}
      {labels.length > 0 && <section className="saved-label-library"><button className="saved-label-library-toggle" type="button" aria-expanded={savedLabelsOpen} onClick={() => setSavedLabelsOpen((current) => !current)}><span><strong>{labels.length} saved package label{labels.length === 1 ? "" : "s"}</strong><small>Automatically reused across Smoothies and Meal Plans.</small></span><em>{savedLabelsOpen ? "Hide saved labels" : "Review saved labels"}</em></button>{savedLabelsOpen && <div className="saved-label-grid">{labels.map((label) => <article key={label.id}><div><strong>{label.brand} · {label.productName}</strong><span>Linked as {label.ingredientName}</span><small>{label.servingAmount} {label.servingUnit} = {label.servingGrams} g · checked {new Date(label.labelCheckedAt).toLocaleDateString()}</small></div><button type="button" onClick={() => remove(label.id)} aria-label={`Remove ${label.productName}`}><Trash2 size={15} /></button></article>)}</div>}</section>}
      {!formOpen && <button className="open-nutrition-label" type="button" onClick={() => { setForm(emptyForm()); setFormOpen(true); setMessage(""); }}><Plus size={17} /> Add another Nutrition Facts label</button>}
      {formOpen && <form onSubmit={submit}>
        <div className="label-identity-grid">
          ...
          <label><span>Serving unit</span><select value={form.servingUnit} onChange={(event) => setForm({ ...form, servingUnit: event.target.value })}><option>scoop</option><option>cup</option><option>tbsp</option><option>tsp</option><option>pinch</option><option>piece</option><option>slice</option><option>can</option><option>g</option><option>oz</option></select></label>
        ...
        <div className="nutrition-label-actions"><button className="save-nutrition-label" type="submit"><Plus size={17} /> Validate and save Nutrition Facts</button><button className="cancel-nutrition-label" type="button" onClick={() => { setForm(emptyForm()); setFormOpen(false); setMessage(""); }}>Cancel</button></div>
        {message && <p className="label-registry-message" role="status">{message}</p>}
      </form>}
      {!formOpen && message && <p className="label-registry-message" role="status">{message}</p>}
    </div>
  </details>;
}
